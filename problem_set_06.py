...
def select_match(player_1, mode, player_2=None, game_type =None, stage=None):
    ...
    if mode.lower() == 'smash':
        if stage:
            if game_type:
                start_smash(stage, game_type)
                return_value = (f"Stage: {stage}", f"Game Type: {game_type}")
                return return_strs + return_value
            start_smash(stage)
            return_value = (f"Stage: {stage}",)
            return return_strs + return_value
        elif game_type:
            start_smash(None, game_type)
            return_value = (f"Game Type: {game_type}",)
            return return_strs + return_value
        else:
            start_smash()
            return_value = None
...
def main():
    ...
    smash_bros_matches = []
    smash_bros_matches.append(select_match("Yoshi", "smash", None, None, "Paper Mario"))
    smash_bros_matches.append(select_match("Sonic", "smash", "Peach", None, "Smashville"))
    smash_bros_matches.append(select_match("Waluigi", "smash", None, "Special Smash", "Smashville"))
    smash_bros_matches.append(select_match("Zelda", "classic", None, "training", None))
    smash_bros_matches.append(select_match("Kirby", "classic", "King K. Rool", None, None))
